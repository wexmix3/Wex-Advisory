...
// ── Colors (solid hex only — pdfkit ignores alpha suffix) ─────────────────────
const NAVY  = "#0F1F3D";
const GOLD  = "#C8A84B";
const WHITE = "#FFFFFF";
const GRAY  = "#4A4A5A";
...
const RED   = "#B83232";
const AMBER = "#C07020";
// Muted tones for dark (navy) backgrounds — no alpha tricks
const DMID  = "#8899BB";   // subtitles on dark bg
const DLOW  = "#4A6080";   // faint labels on dark bg
const DTILE = "#162847";   // tile/card bg on dark bg
const DLINE = "#1E3358";   // thin rules on dark bg
...
function hdr(label) {
  ...
  doc.fillColor(LGRAY).text("WEX ADVISORY  |  CONFIDENTIAL", 0, 7,
...
function ftr(label) {
  const savedY = doc.y;
  doc.moveTo(ML, H - 26).lineTo(W - MR, H - 26).lineWidth(0.4).strokeColor(RULE).stroke();
  doc.fontSize(6.5).font("Helvetica").fillColor(LGRAY).text(label, ML, H - 16, { lineBreak: false });
  doc.font("Helvetica-Bold").fillColor(LGRAY)
    .text("CONFIDENTIAL", 0, H - 16, { width: W - MR, align: "right", lineBreak: false });
  doc.y = savedY;
}

function secHead(num, title) {
  const y = doc.y;
  doc.fontSize(58).font("Helvetica-Bold").fillColor("#EAECF4")
    .text(String(num).padStart(2, "0"), ML - 4, y - 4, { lineBreak: false });
  doc.fontSize(20).font("Helvetica-Bold").fillColor(NAVY)
    .text(title, ML + 46, y + 8, { lineBreak: false });
  doc.y = y + 44;
...
function body(text, opts) {
  doc.fontSize(9.5).font("Helvetica").fillColor(GRAY)
    .text(text, ML, doc.y, { width: CW, lineGap: 1.5, ...opts });
  doc.y += 7;
}

function sub(text) {
  doc.y += 5;
  doc.fontSize(10.5).font("Helvetica-Bold").fillColor(NAVY).text(text, ML, doc.y);
  doc.y += 8;
}

function bullet(text, indent) {
  const x = indent || ML + 10;
  doc.fontSize(9).font("Helvetica").fillColor(GRAY)
    .text("›  " + text, x, doc.y, { width: CW - (x - ML), lineGap: 1.5 });
  doc.y += 4;
}

function measH(text, width, fontSize, font) {
  doc.font(font || "Helvetica").fontSize(fontSize || 9);
...
doc.fontSize(8).font("Helvetica-Bold").fillColor(GOLD)
  .text("WEX ADVISORY", ML, 24, { characterSpacing: 2.5, lineBreak: false });
doc.fontSize(7).font("Helvetica-Bold").fillColor(DLOW)
  .text("CONFIDENTIAL", 0, 24, { width: W - MR, align: "right", characterSpacing: 1.2, lineBreak: false });
...
doc.fontSize(42).font("Helvetica-Bold").fillColor(WHITE).text("Meridian Workspace", ML, cy + 18);
doc.fontSize(13).font("Helvetica").fillColor(DMID).text("Austin, TX", ML, cy + 70);
doc.moveTo(ML, cy + 92).lineTo(ML + 48, cy + 92).lineWidth(2).strokeColor(GOLD).stroke();
doc.fontSize(10).font("Helvetica").fillColor(DLOW)
  .text("Competitor Analysis  \xb7  Market Sizing  \xb7  Strategic Recommendations", ML, cy + 104);
...
tiles.forEach((t, i) => {
  ...
  const ty = 538 + Math.floor(i / 3) * 22;
  doc.rect(tx, ty, tw, 16).fill(DTILE);
  doc.fontSize(7).font("Helvetica").fillColor(DLOW).text(t, tx + 8, ty + 4, { lineBreak: false });
});

doc.moveTo(ML, H - 42).lineTo(W - MR, H - 42).lineWidth(0.4).strokeColor(DLINE).stroke();
doc.fontSize(7.5).font("Helvetica").fillColor(DLOW)
  .text("Prepared May 2025  \xb7  Wex Advisory  \xb7  wexadvisory.com", ML, H - 30);
...
const cTH = Math.max(...cardData.map(c => doc.heightOfString(c.txt, { width: cw3 - 20, lineGap: 1.5 })));
const ch = cTH + 32;
const cardsY = doc.y;
cardData.forEach((c, i) => {
  ...
  doc.fontSize(6.5).font("Helvetica-Bold").fillColor(GOLD)
    .text(c.lbl, cx + 10, cardsY + 10, { characterSpacing: 0.7, lineBreak: false });
  doc.fontSize(8.5).font("Helvetica").fillColor(GRAY)
    .text(c.txt, cx + 10, cardsY + 23, { width: cw3 - 20, lineGap: 1.5 });
});
doc.y = cardsY + ch + 12;

const stats = [
  { v: "6.4", lbl: "Market Position\nScore" },
  { v: "5",   lbl: "Competitor\nProfiles" },
  { v: "3.4x",lbl: "Traffic Gap vs.\nIndustrious" },
  { v: "3",   lbl: "Immediate\nPriority Actions" },
];
const stw = (CW - 12) / 4, sth = 58, statsY = doc.y;
stats.forEach((s, i) => {
  const sx = ML + i * (stw + 4);
  doc.rect(sx, statsY, stw, sth).fill(CREAM);
  doc.rect(sx, statsY, stw, sth).strokeColor(RULE).lineWidth(0.5).stroke();
  doc.fontSize(26).font("Helvetica-Bold").fillColor(NAVY)
    .text(s.v, sx, statsY + 7, { width: stw, align: "center", lineBreak: false });
  doc.fontSize(7).font("Helvetica").fillColor(LGRAY)
    .text(s.lbl, sx, statsY + 40, { width: stw, align: "center" });
});
doc.y = statsY + sth + 12;
...
toc.forEach(([num, title, pg], i) => {
  const ty = doc.y;
  doc.rect(ML, ty, CW, 34).fill(i % 2 === 0 ? CREAM : WHITE);
  doc.fontSize(18).font("Helvetica-Bold").fillColor(i < 6 ? "#DDDDE8" : "#EEEEEE")
    .text(num, ML + 8, ty + 7, { lineBreak: false });
  doc.fontSize(10.5).font("Helvetica-Bold").fillColor(NAVY)
    .text(title, ML + 46, ty + 11, { lineBreak: false });
  doc.fontSize(9.5).font("Helvetica").fillColor(LGRAY)
    .text(pg, 0, ty + 12, { width: W - MR, align: "right", lineBreak: false });
  doc.y = ty + 34;
});
ftr("Wex Advisory  |  Meridian Workspace  |  May 2025");
...
profile.forEach((row, i) => {
  const ry = doc.y;
  doc.rect(ML, ry, 158, 19).fill(i % 2 === 0 ? CREAM : WHITE);
  doc.rect(ML + 158, ry, CW - 158, 19).fill(WHITE);
  doc.rect(ML, ry, CW, 19).strokeColor(RULE).lineWidth(0.4).stroke();
  ...
  doc.fontSize(9).font("Helvetica").fillColor(NAVY)
    .text(row[1], ML + 166, ry + 5, { lineBreak: false });
  doc.y = ry + 19;
});
...
const comps = [
  ...
  },
  {
    ...
    meta: { URL: "regus.com/austin", Location: "6 locations across Austin metro", Pricing: "$150–$350/desk/mo", Rating: "3.8 / 5 (avg. across 6 locations)", Traffic: "~1,400 visits/mo" },
...
comps.forEach((comp, ci) => {
  ...
  const hy = doc.y;
  doc.rect(ML, hy, CW, 40).fill(NAVY);
  doc.rect(ML, hy, 40, 40).fill(GOLD);
  doc.fontSize(15).font("Helvetica-Bold").fillColor(NAVY)
    .text(String(ci + 1), ML, hy + 12, { width: 40, align: "center", lineBreak: false });
  doc.fontSize(14).font("Helvetica-Bold").fillColor(WHITE)
    .text(comp.name, ML + 50, hy + 5, { lineBreak: false });
  doc.fontSize(7).font("Helvetica").fillColor(DMID)
    .text(comp.cat.toUpperCase(), ML + 50, hy + 25, { characterSpacing: 0.4, lineBreak: false });
  doc.y = hy + 46;

  // Meta row
  const metaKeys = Object.keys(comp.meta);
  const mw = CW / metaKeys.length, my = doc.y;
  doc.rect(ML, my, CW, 42).fill(CREAM);
  doc.rect(ML, my, CW, 42).strokeColor(RULE).lineWidth(0.4).stroke();
  metaKeys.forEach((k, mi) => {
    const mx = ML + mi * mw;
    if (mi > 0) doc.moveTo(mx, my + 5).lineTo(mx, my + 37).lineWidth(0.4).strokeColor(RULE).stroke();
    doc.fontSize(6.5).font("Helvetica-Bold").fillColor(LGRAY)
      .text(k.toUpperCase(), mx + 7, my + 8, { characterSpacing: 0.4, width: mw - 14, lineBreak: false });
    doc.fontSize(8).font("Helvetica-Bold").fillColor(NAVY)
      .text(comp.meta[k], mx + 7, my + 22, { width: mw - 14 });
  });
  doc.y = my + 50;

  // Description
  doc.fontSize(9.5).font("Helvetica").fillColor(GRAY)
    .text(comp.desc, ML, doc.y, { width: CW, lineGap: 1.5 });
  doc.y += 12;
  ...
  doc.rect(ML, labY, colW, 20).fill(NAVY);
  doc.fontSize(7).font("Helvetica-Bold").fillColor(WHITE)
    .text("STRENGTHS", ML + 10, labY + 6, { characterSpacing: 0.5, lineBreak: false });
  doc.rect(ML + colW + 12, labY, colW, 20).fill(RED);
  doc.fontSize(7).font("Helvetica-Bold").fillColor(WHITE)
    .text("WEAKNESSES", ML + colW + 22, labY + 6, { characterSpacing: 0.5, lineBreak: false });
  ...
  comp.weaknesses.forEach(w => {
    doc.fontSize(9).font("Helvetica").fillColor(GRAY)
      .text("›  " + w, ML + colW + 20, rY, { width: colW - 16, lineGap: 1.5 });
    rY = doc.y + 4;
    doc.y = rY;
  });

  doc.y = Math.max(endLY, rY) + 6;
...
const gaps = [
  { p: "HIGH",   c: RED,   title: "Search & Digital Discoverability",
    body: "Meridian holds zero page-one Google rankings for any Austin coworking keyword. Industrious, Capital Factory, and WeWork collectively dominate the top 8 organic positions. Without paid search or SEO remediation, Meridian is invisible to the highest-intent buyers at the moment of decision." },
  { p: "HIGH",   c: RED,   title: "Pricing Transparency",
    body: "No pricing page exists on the Meridian website. Research shows 73% of coworking prospects self-qualify on price before contacting an operator. The absence of a pricing page forces prospective members to contact competitors who publish pricing openly — this is the single highest-friction barrier in Meridian’s funnel." },
  { p: "HIGH",   c: RED,   title: "Review Volume on Google Maps",
    body: "47 Google reviews vs. a competitor average of 214. Review volume is a primary factor in Google Maps local pack ranking. Meridian’s 4.6 rating is excellent, but with fewer than 50 reviews, Google classifies the profile as low-confidence and ranks it outside the local pack for most target terms." },
  { p: "MEDIUM", c: AMBER, title: "Day Pass Product Visibility",
    body: "Day passes are reportedly available but not discoverable — no landing page, no Google Business listing, no published pricing. Transient corporate users represent an estimated $40,000–$80,000/year revenue opportunity currently defaulting to WeWork and Industrious, who have published and marketed day pass products." },
  { p: "MEDIUM", c: AMBER, title: "Event & Community Merchandising",
    body: "Meridian’s events calendar is one of the strongest in the Austin market, averaging 6 events per month. However, the website does not display upcoming or past events. This is the brand’s clearest differentiator versus corporate operators and it is entirely invisible to prospective members during the consideration phase." },
  { p: "MEDIUM", c: AMBER, title: "LinkedIn Organic Presence",
    body: "Meridian’s LinkedIn company page has 310 followers with no consistent posting cadence. Industrious Austin posts 2–4x per week. LinkedIn is the primary channel through which corporate buyers discover and evaluate coworking spaces — Meridian has no presence at the point of decision for its highest-value buyer segment." },
];

gaps.forEach(g => {
  const gy = doc.y;
  doc.rect(ML, gy, 50, 17).fill(g.c);
  doc.fontSize(6.5).font("Helvetica-Bold").fillColor(WHITE)
    .text(g.p, ML, gy + 5, { width: 50, align: "center", characterSpacing: 0.5, lineBreak: false });
  doc.fontSize(10.5).font("Helvetica-Bold").fillColor(NAVY)
    .text(g.title, ML + 58, gy + 2, { lineBreak: false });
  doc.y = gy + 20;
  doc.fontSize(9).font("Helvetica").fillColor(GRAY)
...
// Score badge
const sbY = doc.y;
doc.rect(ML, sbY, 94, 72).fill(NAVY);
doc.fontSize(34).font("Helvetica-Bold").fillColor(GOLD)
  .text("6.4", ML, sbY + 10, { width: 94, align: "center", lineBreak: false });
doc.fontSize(7).font("Helvetica-Bold").fillColor(DMID)
  .text("OUT OF 10", ML, sbY + 52, { width: 94, align: "center", characterSpacing: 0.5, lineBreak: false });
doc.fontSize(9.5).font("Helvetica-Bold").fillColor(NAVY)
  .text("MARKET POSITION SCORE", ML + 104, sbY + 8, { lineBreak: false });
doc.fontSize(9).font("Helvetica").fillColor(GRAY)
  .text("Score reflects 7 weighted dimensions: product quality, pricing, digital presence, community, location, review profile, and brand equity. Assessed vs. 5 direct competitors as of May 2025.", ML + 104, sbY + 26, { width: CW - 112, lineGap: 1.5 });
doc.y = sbY + 82;
...
const hwidths = [136, 55, 66, 55, 70, 68, 54];

let tx = ML, ty = doc.y;
hcols.forEach((col, ci) => {
  const cw = hwidths[ci];
  doc.rect(tx, ty, cw, 20).fill(NAVY);
  doc.fontSize(6.5).font("Helvetica-Bold").fillColor(ci === 1 ? GOLD : WHITE)
    .text(col, tx + 4, ty + 6, { width: cw - 8, align: ci > 0 ? "center" : "left", lineBreak: false, characterSpacing: 0.2 });
  tx += cw;
});
ty += 20;
...
dims.forEach((row, ri) => {
  ...
  [row.d, row.mer, row.ind, row.wew, row.com, row.cap, row.reg].forEach((val, ci) => {
    const cw = hwidths[ci];
    doc.rect(tx, ty, cw, 20).fill(rbg);
    doc.rect(tx, ty, cw, 20).strokeColor(RULE).lineWidth(0.3).stroke();
    const isHigh = typeof val === "number" && val >= 8 && !row.total;
    const tc = row.total
      ? (ci === 0 ? WHITE : ci === 1 ? GOLD : DMID)
      : (ci === 0 ? NAVY : isHigh ? "#1a7a3a" : GRAY);
    doc.fontSize(ci === 0 ? 8 : 9).font(row.total || isHigh ? "Helvetica-Bold" : "Helvetica").fillColor(tc)
      .text(String(val), tx + 4, ty + 5, { width: cw - 8, align: ci > 0 ? "center" : "left", lineBreak: false });
    tx += cw;
  });
  ty += 20;
});
...
const ph1Y = doc.y;
doc.rect(ML, ph1Y, CW, 22).fill(NAVY);
doc.fontSize(7.5).font("Helvetica-Bold").fillColor(GOLD)
  .text("PHASE 1  |  IMMEDIATE  (0–30 DAYS)", ML + 12, ph1Y + 7, { characterSpacing: 0.5, lineBreak: false });
doc.y = ph1Y + 30;

const recs1 = [
  { num: "01", title: "Launch a Pricing Page",
  ...
  { num: "02", title: "Google Business Optimization",
  ...
  { num: "03", title: "Day Pass Product Page",
...
recs1.forEach(r => {
  const ry = doc.y;
  doc.rect(ML, ry, 34, 34).fill(GOLD);
  doc.fontSize(12).font("Helvetica-Bold").fillColor(NAVY)
    .text(r.num, ML, ry + 9, { width: 34, align: "center", lineBreak: false });
  doc.fontSize(10.5).font("Helvetica-Bold").fillColor(NAVY)
    .text(r.title, ML + 42, ry + 3, { lineBreak: false });
  doc.y = ry + 20;
  doc.fontSize(9.5).font("Helvetica").fillColor(GRAY)
    .text(r.body, ML + 42, doc.y, { width: CW - 42, lineGap: 1.5 });
  doc.y += 5;
  doc.fontSize(8).font("Helvetica-Bold").fillColor(GOLD)
    .text(r.target, ML + 42, doc.y, { lineBreak: false });
  doc.y += 12;
  rule();
});
...
// ═══════════════════════════════════════════════════════════════════════════════
// PAGE 14 — 06 STRATEGIC RECOMMENDATIONS Phase 2
// ═══════════════════════════════════════════════════════════════════════════════
doc.addPage(); hdr("06  Strategic Recommendations");
doc.y = 38;

const ph2Y = doc.y;
doc.rect(ML, ph2Y, CW, 22).fill(NAVY);
doc.fontSize(7.5).font("Helvetica-Bold").fillColor(GOLD)
  .text("PHASE 2  |  SHORT-TERM  (30–90 DAYS)", ML + 12, ph2Y + 7, { characterSpacing: 0.5, lineBreak: false });
doc.y = ph2Y + 30;
...
recs2.forEach(r => {
  const ry = doc.y;
  doc.rect(ML, ry, 34, 34).fill(NAVY);
...
doc.y += 4;
const vY = doc.y;
doc.rect(ML, vY, CW, 76).fill(CREAM);
doc.rect(ML, vY, 4, 76).fill(GOLD);
doc.fontSize(7).font("Helvetica-Bold").fillColor(GOLD)
  .text("LONG-TERM VISION  (6–18 MONTHS)", ML + 14, vY + 9, { characterSpacing: 0.5, lineBreak: false });
doc.fontSize(9.5).font("Helvetica").fillColor(GRAY)
  .text("Pursue a second Austin location in the East 6th or SoCo corridor to capture demand from non-downtown-centric professionals. Simultaneously, develop an enterprise outreach program targeting 25–100 person Austin tech companies seeking satellite offices — this segment is underserved by both boutique and national operators and represents $15,000–$40,000/month in recurring revenue per account won.", ML + 14, vY + 22, { width: CW - 22, lineGap: 1.5 });
doc.y = vY + 86;
...
doc.fontSize(7).font("Helvetica-Bold").fillColor(DLOW)
  .text("CONFIDENTIAL", 0, 24, { width: W - MR, align: "right", characterSpacing: 1.2, lineBreak: false });
doc.fontSize(26).font("Helvetica-Bold").fillColor(WHITE).text("Supplemental Note", ML, 282);
doc.moveTo(ML, 324).lineTo(ML + 44, 324).lineWidth(2).strokeColor(GOLD).stroke();
doc.fontSize(11).font("Helvetica").fillColor(DMID).text("Key Concepts & Data Sources", ML, 336);
doc.fontSize(8).font("Helvetica").fillColor(DLOW).text("Meridian Workspace  \xb7  May 2025", ML, 358);

// ═══════════════════════════════════════════════════════════════════════════════
// KEY CONCEPTS — packed into pages (multiple per page)
// ═══════════════════════════════════════════════════════════════════════════════
const concepts = [
...
concepts.forEach(c => {
  // Measure with font explicitly set
  doc.font("Helvetica-Bold").fontSize(10.5);
  const termH = doc.heightOfString(c.term, { width: CW - 18 });
  doc.font("Helvetica").fontSize(9);
  const defH = doc.heightOfString(c.def, { width: CW - 18, lineGap: 1.5 });
  const entH = termH + defH + 20; // gold bar top + gap between term/def + rule gap

  const needsPage = concFirst || (doc.y + entH > BOTTOM);
  if (needsPage) {
    doc.addPage();
    hdr("Supplemental Note  |  Key Concepts");
    if (concFirst) {
      doc.fontSize(16).font("Helvetica-Bold").fillColor(NAVY).text("Key Concepts", ML, doc.y);
      doc.y += 12;
      doc.moveTo(ML, doc.y).lineTo(W - MR, doc.y).lineWidth(1.5).strokeColor(GOLD).stroke();
      doc.y += 10;
      concFirst = false;
  ...
  const ey = doc.y;
  const barH = termH + defH + 14;
  doc.rect(ML, ey, 3, barH).fill(GOLD);
  doc.fontSize(10.5).font("Helvetica-Bold").fillColor(NAVY)
    .text(c.term, ML + 12, ey, { width: CW - 14, lineBreak: false });
  doc.y = ey + termH + 4;
  doc.fontSize(9).font("Helvetica").fillColor(GRAY)
    .text(c.def, ML + 12, doc.y, { width: CW - 18, lineGap: 1.5 });
  doc.y += 8;
  rule();
...
// ═══════════════════════════════════════════════════════════════════════════════
// DATA SOURCES — on same page if room, else new page
// ═══════════════════════════════════════════════════════════════════════════════
if (doc.y + 180 > BOTTOM) {
  doc.addPage();
  hdr("Supplemental Note  |  Data Sources");
  doc.y = 38;
}
...
sources.forEach(s => {
  ...
  doc.fontSize(9.5).font("Helvetica-Bold").fillColor(NAVY).text(s.cat, ML, doc.y);
  doc.y += 11;
...
const dY = doc.y;
doc.rect(ML, dY, CW, 84).fill(CREAM);
doc.rect(ML, dY, CW, 84).strokeColor(RULE).lineWidth(0.5).stroke();
doc.fontSize(7).font("Helvetica-Bold").fillColor(GOLD)
  .text("IMPORTANT DISCLOSURE", ML + 12, dY + 10, { characterSpacing: 0.6, lineBreak: false });
doc.fontSize(8.5).font("Helvetica").fillColor(GRAY)
  .text("This report is based on publicly available information, third-party data sources, and independent analysis by Wex Advisory. Traffic figures and competitive scores are estimates and should be treated as directional indicators rather than precise measurements. Market conditions change rapidly; data reflects observations made in April–May 2025. This report does not constitute legal, financial, or investment advice and should be used for internal strategic planning purposes only. All information you provide during a paid engagement is held in strict confidence and is not shared with third parties.", ML + 12, dY + 24, { width: CW - 24, lineGap: 1.8 });

ftr("Supplemental Note  |  Wex Advisory  |  Meridian Workspace  |  May 2025");

doc.end();
console.log("Done — written to public/sample-report.pdf");
